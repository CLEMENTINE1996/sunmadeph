...
import { ChevronDown, MapPin, Briefcase, Users, Star } from 'lucide-react'; 
                                        ...
                                        {isExpanded && (
                                            ...
                                            >
                                                <div className="px-8 pb-10 grid grid-cols-1 md:grid-cols-2 gap-12 border-t border-dark/5 dark:border-light/5 pt-10">
                                                    ...
                                                    <div>
                                                        <h5 className="text-[10px] uppercase font-black tracking-[0.2em] text-accent dark:text-accentDark mb-4">Branch Profile</h5>
                                                        <p className="text-dark/70 dark:text-light/70 leading-relaxed mb-8 italic">
                                                            "{branch.description}"
                                                        </p>
                                                        
                                                        <h5 className="text-[10px] uppercase font-black tracking-[0.2em] text-accent dark:text-accentDark mb-4">Available Services</h5>
                                                        <div className="grid grid-cols-1 gap-3">
                                                            {branch.services.map((service, idx) => (
                                                                <div key={idx} className="flex items-center gap-3 text-sm font-medium text-dark/80 dark:text-light/80 bg-[#FBFBFB] dark:bg-white/5 p-3 rounded-sm border border-dark/5">
                                                                    <Star size={14} className="text-accent dark:text-accentDark" />
                                                                    {service}
                                                                </div>
                                                            ))}
                                                        </div>
                                                    ...
                                                    <div className="bg-[#FBFBFB] dark:bg-white/5 p-8 rounded-sm">
                                                        <div className="flex items-center gap-2 mb-6">
                                                            <Users size={18} className="text-accent dark:text-accentDark" />
                                                            <h5 className="text-[10px] uppercase font-black tracking-[0.2em] text-dark/40 dark:text-light">Branch Personnel</h5>
                                                        </div>
                                                        <ul className="space-y-4">
                                                            {branch?.branch_manager?.staff.map((staff, idx) => (
                                                                <li key={idx} className="flex flex-col border-b border-dark/5 pb-3 last:border-0">
                                                                    <span className="font-bold text-dark dark:text-light">{staff.name}</span>
                                                                    <span className="text-[10px] uppercase tracking-widest text-accent dark:text-accentDark font-semibold">{staff.role}</span>
                                                                </li>
                                                            ))}
                                                        </ul>
                                                    </div>
